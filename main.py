...
from code.training import prepare_training
...
def run_block(block, config, win, stimulus_all, stimulus_type, fixation, clock, block_type, block_idx):
    global N, RESULTS
    stimulus_last = []
...
def main():
    ...
    for block_idx, [block, block_type] in enumerate(zip(blocks, block_order)):
        # training
        show_info(win, join('.', 'messages', f'block_{block_type}_training.txt'), text_color=config["text_color"],
                  text_size=config["text_size"], screen_res=screen_res)
        training_trials = prepare_training(trials, config["training_elements_per_group"])
        run_block(training_trials, config, win, stimulus_all, stimulus_type,
                  fixation, clock, block_type+"_training", block_idx)

        # experiment
        show_info(win, join('.', 'messages', f'block_{block_type}.txt'), text_color=config["text_color"],
                  text_size=config["text_size"], screen_res=screen_res)
        run_block(block, config, win, stimulus_all, stimulus_type, fixation, clock, block_type, block_idx)
